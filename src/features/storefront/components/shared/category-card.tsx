  ...
  return (
    ...
    >
      <Link href={`/category/${category.slug}`} className="flex flex-col items-center gap-4">
        <div className="relative w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden shadow-md group-hover:shadow-xl transition-shadow bg-slate-50 border-4 border-white">
          <Image
        ...
        <h3 className="font-bold text-slate-800 text-center transition-colors group-hover:text-(--color-primary)">
          {category.name}
        </h3>
      </Link>
    </motion.div>
